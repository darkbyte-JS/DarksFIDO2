...
public sealed class WebAuthnService
{
    ...
    public FidoKeyRecord RegisterCredential(IntPtr ownerWindow, Guid profileId, string displayName, bool usePlatformAuthenticator = false)
    {
        ...
        try
        {
            ...
            if (attestation.cbCredentialId is < 1 or > MaximumCredentialIdBytes || attestation.pbCredentialId == IntPtr.Zero ||
                ...
                attestation.Extensions.cExtensions > 1024)
    ...
    public uint VerifyCredential(IntPtr ownerWindow, FidoKeyRecord credential)
    {
        ...
        try
        {
            ...
            if (assertion.cbAuthenticatorData is < 37 or > CtapCbor.MaximumDocumentBytes || assertion.pbAuthenticatorData == IntPtr.Zero ||
                ...
                assertion.Extensions.cExtensions > 1024)
        ...
        ReadOnlySpan<byte> expectedCredentialId)
    {
        ...
        ReadOnlySpan<byte> remainder = authenticatorData[(55 + credentialLength)..];
        ReadOnlySpan<byte> publicKeyCose;
        if ((authenticatorData[32] & ExtensionDataFlag) != 0)
        {
            CtapCbor.DecodeFirst(remainder, out int consumed);
            if (consumed <= 0 || consumed > remainder.Length)
                throw new CryptographicException("The registration authenticator data has an invalid public key structure.");
            publicKeyCose = remainder[..consumed];
        }
        else
        {
            publicKeyCose = remainder;
        }
    ...
    private static void ValidateAuthenticatorData(ReadOnlySpan<byte> authenticatorData, bool requireAttestedCredentialData)
    {
        ...
        if ((flags & BackupStateFlag) != 0 && (flags & BackupEligibleFlag) == 0)
        ...
        if (!requireAttestedCredentialData)
        {
            if ((flags & ExtensionDataFlag) != 0)
            {
                if (authenticatorData.Length <= 37)
                    throw new CryptographicException("The assertion authenticator data is missing extension payload.");
            }
            else if (authenticatorData.Length != 37)
            {
