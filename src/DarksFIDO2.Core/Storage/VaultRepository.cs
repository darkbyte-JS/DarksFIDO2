...
public sealed class VaultRepository
{
        ...
        lock (_gate)
        {
            ...
            if (File.Exists(_paths.IndexPath))
            {
                try
                {
                    ...
                    try
                    {
                        index = JsonSerializer.Deserialize<ProfileIndex>(json, VaultCryptography.JsonOptions) ?? new ProfileIndex();
                        ...
                        for (int i = 0; i < index.Profiles.Count; i++)
                        {
                            ...
                            {
                                // Retain summary if metadata cannot be re-read
                            }
                        }
                    }
                    finally
            ...
            // Dynamically discover any profile directories on disk not present in the index
            if (Directory.Exists(_paths.ProfilesDirectory))
            {
                bool indexUpdated = false;
                foreach (string dir in Directory.EnumerateDirectories(_paths.ProfilesDirectory))
                {
                    string dirName = Path.GetFileName(dir);
                    if (Guid.TryParse(dirName, out Guid profileId) || Guid.TryParseExact(dirName, "N", out profileId))
                    {
                        if (!index.Profiles.Any(p => p.Id == profileId))
                        {
                            try
                            {
                                ProfileMetadata metadata = ReadMetadata(profileId);
                                index.Profiles.Add(new ProfileSummary(metadata.Id, metadata.Name, metadata.KeyfileRequired, metadata.CreatedUtc));
                                indexUpdated = true;
                            }
                            catch
                            {
                                // Ignore non-profile or unreadable directories
                            }
                        }
                    }
                }
                ...
                if (indexUpdated)
                {
                    try
                    {
                        if (index.LastProfileId == null && index.Profiles.Count > 0)
                        {
                            index.LastProfileId = index.Profiles[0].Id;
                        }
                        WriteIndex(index);
                    }
                    catch
    ...
    public UnlockedProfile CreateProfile(ProfileCreationRequest request)
    {
        ...
        if (request.Pin.Length < 6) throw new ArgumentException("New profiles require a PIN/passphrase of at least 6 characters.");
    ...
    public void ChangePin(UnlockedProfile profile, string newPin, byte[]? currentKeyfile, string? newRecoveryCode = null)
    {
        ...
        if (newPin.Length < 6) throw new ArgumentException("The new PIN/passphrase must contain at least 6 characters.", nameof(newPin));
